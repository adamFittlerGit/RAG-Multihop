...
if __name__ == '__main__':
  # Set up argument parser for just the rank model name
  parser = argparse.ArgumentParser(description='Run ranking and reranking process.')
  parser.add_argument('--rank_model_name', type=str, required=True,
                    help='Name of the ranking model to use')
  ...
  output_name = f"output/stage-one/rerankerB/{rank_model_name.split('/')[-1]}--{rerank_model_name.split('/')[-1]}.json"
